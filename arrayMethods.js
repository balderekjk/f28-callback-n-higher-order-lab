...
// CODE HERE
const totalPopulation = populations.reduce((a, c) => (c += a));
...
// CODE HERE
const showTruePrice = orders.map(item => {
  return item.price * (1 + item.tax);
});

// console.log(showTruePrice);
...
let bobsPricesArr = [];
const bobsPrices = purchases.filter(data => {
  if (data.owner === 'Bob') {
    bobsPricesArr.push(data.price);
  }
});
const bobsTotal = bobsPricesArr.reduce((a, c) => (c += a));
